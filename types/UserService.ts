...
export class UserService {
  ...
  public static async getAllProducts(limit: number): Promise<ITypes[]> {
    const response = await axios.get<{ products: ITypes[] }>(`${this.BASE_URL}?limit=${limit}`);
  ...
  public static async getProductsByRating(rating: number, limit: number): Promise<ITypes[]> {
    const response = await axios.get<{ products: ITypes[] }>(`${this.BASE_URL}?limit=${limit}`);
    const products = response.data.products;
    const sortedProducts = products.sort((a, b) => b.rating - a.rating);
    const filteredProducts = sortedProducts.filter((product) => product.rating === rating);
